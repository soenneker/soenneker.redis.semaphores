...
public sealed partial class RedisSemaphore
{
        ...
        TimeSpan leaseDuration, CancellationToken cancellationToken = default)
    {
        ...
        if (maxCount < 1 || leaseDuration < TimeSpan.FromMilliseconds(1))
            throw new ArgumentOutOfRangeException(nameof(maxCount));
        var token = Guid.NewGuid().ToString("N");
        int start = RandomUtil.Next(maxCount);
        for (var offset = 0; offset < maxCount; offset++)
        {
            cancellationToken.ThrowIfCancellationRequested();
            var slot = (int)(((long)start + offset) % maxCount);
            var key = $"{keyPrefix}:semaphore:{slot}";
    ...
    public static Task<bool> RenewPermit(IDatabase database, RedisSemaphorePermit permit, TimeSpan duration,
        ...
        RedisAtomics.CompareExpire(database, permit.Key, permit.Token, duration, cancellationToken);
    ...
    public static Task<bool> ReleasePermit(IDatabase database, RedisSemaphorePermit permit,
        CancellationToken cancellationToken = default) =>
        RedisAtomics.CompareDelete(database, permit.Key, permit.Token, cancellationToken);
}